...
def write_data(of, json_data):
   of.write('{}\t{}\t'.format(json_data['chrom'].replace('chr', ''), json_data['start']).encode())
   #json.dump(json_data, of, separators = (b',', b':'))
   of.write(json.dumps(json_data, separators = (',', ':')).encode())
   of.write(b'\n')
